...
enrich<-function(categoria){
  stages<-levels(vsd$group)
  significant_GO_categoria <- data.frame()
  for (clustNum in stages){  
    sub_cluster <- subset(vsd, vsd$group == clustNum)
    topVarGenes <- head(order(rowVars(assay(sub_cluster)), decreasing = TRUE), 500)
    ...
    myGOdata1 <- new("topGOdata", description=paste0("GO top 500 variable", clustNum), ontology=categoria, allGenes=geneList1,  annot = annFUN.gene2GO, gene2GO = geneID2GO1)
    ...
    write.table(allRes1, file = paste0("GO_top_500_variable_", categoria, "_", clustNum, "_all.txt"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
    ...
    write.table(significant_GO_categoria, file = paste0("GO_top_500_variable_",categoria,"_significant_", clustNum, ".txt"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
  }
  return(significant_GO_categoria)
}


PLOTgo<-rbind(enrich_MF, enrich_BP)
head(PLOTgo) 

ggplot(data=BP, aes(x=Term, y=Significant)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  coord_flip()  + facet_grid(~stage)
