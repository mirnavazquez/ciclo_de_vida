...
for (clustNum in c(1:7)){  
  sub_cluster <- subset(data.val4, som_cluster == clustNum & data.val4$coding == "long_non_coding")
  sub_data <- sub_cluster[,c(7:10,17)] # just the sample types
  #colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Adult male", "Adult female", "Sequences")
  ...
  ggsave(paste0("Cluster_cuttree_long_non_coding", clustNum, ".pdf"))
}



for (clustNum in c(1:7)){  
  sub_cluster <- subset(data.val4, som_cluster == clustNum & data.val4$coding == "coding")
  sub_data <- sub_cluster[,c(7:10,17)] # just the sample types
  #colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Adult male", "Adult female", "Sequences")
  colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Sequences")
  m.data <- melt(sub_data)
  ...
  p + geom_point(alpha = 0.5,position = "jitter", size = 0.5, color = "#C4961A") + 
    ...
    xlab("Life stage") +
    ylab("Scaled Gene Expression") +
    ggsave(paste0("Cluster_cuttree_coding", clustNum, ".pdf"))
}

for (clustNum in c(1:7)){  
  sub_cluster <- subset(data.val4, som_cluster == clustNum & data.val4$coding == "TE")
  sub_data <- sub_cluster[,c(7:10,17)] # just the sample types
  #colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Adult male", "Adult female", "Sequences")
  colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Sequences")
  m.data <- melt(sub_data)
  ...
  p + geom_point(alpha = 0.5,position = "jitter", size = 0.5, color = "#C4961A") + 
    ...
    xlab("Life stage") +
    ylab("Scaled Gene Expression") +
    ggsave(paste0("Cluster_cuttree_TE", clustNum, ".pdf"))
}

for (clustNum in c(1:7)){  
  sub_cluster <- subset(data.val4, som_cluster == clustNum & data.val4$coding == "rRNA")
  sub_data <- sub_cluster[,c(7:10,17)] # just the sample types
  #colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Adult male", "Adult female", "Sequences")
  colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Sequences")
  m.data <- melt(sub_data)
  ...
  p + geom_point(alpha = 0.5,position = "jitter", size = 0.5, color = "#C4961A") + 
    ...
    xlab("Life stage") +
    ylab("Scaled Gene Expression") +
    ggsave(paste0("Cluster_cuttree_rRNA", clustNum, ".pdf"))
}

for (clustNum in c(1:7)){  
  sub_cluster <- subset(data.val4, som_cluster == clustNum & data.val4$coding == "tRNA")
  sub_data <- sub_cluster[,c(7:10,17)] # just the sample types
  #colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Adult male", "Adult female", "Sequences")
  colnames(sub_data)<-c("Egg", "Larvae stage 2", "Larvae stage 3", "Pupa", "Sequences")
  m.data <- melt(sub_data)
  ...
  p + geom_point(alpha = 0.5,position = "jitter", size = 0.5, color = "#C4961A") + 
    ...
    xlab("Life stage") +
    ylab("Scaled Gene Expression") +
    ggsave(paste0("Cluster_cuttree_tRNA", clustNum, ".pdf"))
}
